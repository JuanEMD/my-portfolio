const StackCard = ({ id, name, color, letter, className = "", style = {} }) => {

    const cardClasses = `${className ? className + " " : ""} relative bg-white/80 dark:bg-white/[0.04] border border-black/[0.07] dark:border-white/[0.07] rounded-xl pt-4 px-3.5 pb-3.5 overflow-hidden flex flex-col gap-[0.15rem]  cursor-default hover:-translate-y-1 hover:[border-color:var(--accent)] hover:[box-shadow:0_8px_24px_color-mix(in_srgb,var(--accent)_25%,transparent)]`;
    ...
    return (
        <div id={id} className={cardClasses} style={{ "--accent": color, ...style }}>
